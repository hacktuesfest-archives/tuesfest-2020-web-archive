(function ($) {
	...
	$('.projects-button-container').on( 'click', 'span', function() {
		...
		$(this).find("div").css("background-image", "linear-gradient(rgba(18, 107, 176, .7), rgba(18, 107, 176, .7))," + $(this).find("div").css("background-image"))
		$(".news-block").hide();
		$(filterValue).show();
	...
	$('.voting-block').find('button').click(function() {
		...
		if(!$('.selected-projects-container .voting-block').hasClass(className)){
			...
			$('.selected-projects-container')
			   .append(`<div class="${className} voting-block feature-block-two" data-id="${id}">
							<div class="inner-box">
								<h4><a href="javascript:void(0)">${name}</a></h4>
			...
			$('.remove-project').unbind().click(function(){
				...
				const className = '.' + project.attr("class").split(/\s+/)[0].replace("-","-voting-")
				const id = project.attr("data-id")
				$(className).find('button').css("background-color", "#126BB0")
